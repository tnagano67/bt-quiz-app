...
DECLARE
  ...
  v_score INTEGER;          -- 100点満点のパーセンテージスコア
  v_correct_count INTEGER;  -- 正解数（回答生成用）
  v_total_questions INTEGER; -- 実際の出題数
  v_passed BOOLEAN;
...
BEGIN
  -- 各生徒についてループ
  FOR v_student IN SELECT id, current_grade FROM students ORDER BY year, class, number
  LOOP
    -- 生徒のグレードを最初のグレードにリセット
    SELECT grade_name INTO v_current_grade
    ...
    LOOP
      ...
      FROM grade_definitions
      WHERE grade_name = v_current_grade;
      ...
      FROM questions
      WHERE question_id >= (
        SELECT start_id FROM grade_definitions WHERE grade_name = v_current_grade
      )
      AND question_id <= (
        SELECT end_id FROM grade_definitions WHERE grade_name = v_current_grade
      );

      -- 問題が足りない場合は全問題からランダムに取得
      ...
      JOIN questions q ON q.question_id = t.qid;

      -- 出題数
      v_total_questions := array_length(v_selected_ids, 1);

      -- 正解数を決定（約65%の合格率になるよう調整）
      -- pass_score は100点満点のパーセンテージ（例: 80）
      -- 合格に必要な正解数を算出
      v_rand := random();
      IF v_rand < 0.65 THEN
        -- 合格: 合格ラインの正解数 〜 全問正解
      ...
      -- quiz_records に挿入
      INSERT INTO quiz_records (student_id, grade, score, passed, question_ids, student_answers, correct_answers, taken_at)
      VALUES (v_student.id, v_current_grade, v_score, v_passed, v_question_ids, v_student_answers, v_correct_answers, v_taken_at);
      ...
      IF v_passed THEN
        ...
        IF v_consecutive >= v_required_days THEN
          ...
          FROM grade_definitions
          WHERE display_order > (
            SELECT display_order FROM grade_definitions WHERE grade_name = v_current_grade
          )
          ORDER BY display_order ASC
          LIMIT 1;
  ...
  RAISE NOTICE 'シード完了: % 件の受験記録を生成しました',
